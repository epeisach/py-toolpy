...
def chain_res_atom(pdb):
    ...
    for x in fp:
        ...
        if ch !=' ' and ch in dd.keys() and res not in dd[ch].keys() : dd[ch][res]=[]
        if ch !=' ': dd[ch][res].append(x)

    if(id==1):  print('Warning: file (%s) has no ChainID.' %pdb)
    return dd
...
def pdb2cif(xyz):
    ...
    if n<4: # add this table by pdb_extract
        ...
        for x in fp:
            if '_refine.entry_id' in x or '_refine.details' in x : continue
            if '_refine.' in x : refine.append(x)
#        delete_file(out)

        fw=open(cif, 'w')
        for y in cif_list: fw.write(y)
        for y in refine: fw.write(y)
        fw.close()
...
#    pth='/net/data/remediation-alt/ftp-v4.0/pdb/data/structures/divided'
    ...
    if idd=='pdb':
        ...
        if not check_file(10, pdbfile): os.system ("zcat  %s > %s " % (cif, pdbfile)) #for large entry
        return pdbfile, ''
    elif idd=='sf':
...
def distf(x1, x2, cell):
    ...
    d2=(cell[0]*dx)**2 + (cell[1]*dy)**2 + (cell[2]*dz)**2 + \
        2*cell[1]*cell[2]*dy*dz*ca +  2*cell[0]*cell[2]*dx*dz*cb + \
...
##########################################################
def move(finp, fout):
    if os.path.exists(finp): shutil.move(finp, fout)
...
#    x1='set terminal %s large size 1000,800 # 840,640\n' %psfix
    ...
    if len(title)>0: fw.write('set title "%s" \n' %title)
