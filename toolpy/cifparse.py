...
def cifparse(flist, cate):
    ...
    for i in range(0,nline):  #look for the table
        if line[i][:clen] == cate :
            if (i>0 and line[i-1][:5]== "loop_"): loop=1
            start=i
            m=m+1
    ...
    if (loop==0):
        for i in range(start, nline):
            ...
            elif (line[i][0]=='#' or line[i][:5]=='loop_' or
                  (line[i][0]=='_' and cate not in line[i][:clen])):
                break
        ...
        while j<nline: #through list from start position
            if (line[j][0]==';'):
                ...
                continue
            elif (line[j][0]=='#' or line[j][:clen]== "loop_" or
                  (line[j][0]=='_' and '.' in line[j]) ):
                break
            else:
...
def cif2cif(file):
    ...
    for i, x in enumerate(flist):
         if 'data_' in x.lstrip()[:5] and len(x.strip().split('data_')[1])>0 : nblock.append(i)
...
#    print('file= ', file, len(flist), flist)
    ...
    while st<nlen:
        x=flist[st].strip()
        if len(x)>5 and 'data_' in x[:5] : block=x[5:].split()[0]
        
        n,cate,item=check_item(x)
        if n>0: #potentional cif item
            cates,items='', []
            items.append(item)
            if len(items)==1: cates=cate
            if (st==nlen-1):cif.append([block,cates, items])
            
            for j in range(st+1,nlen):
                y=flist[j].strip()
                if len(y)>5 and 'data_' in y[:5] : block=y[5:].split()[0]
...
def check_item(y):
    ...
    if nlen>4 and ss[0]=='_' and '.' in ss[1:]:
        ...
        cate=ss[:m]
        item=ss[m:]
        if m>1 and '.' not in ss[m+1:] and len(cate)>2 and len(item)>0 : n=1
...
#    print set(asym)
    ...
    for x in uch :
        if len(x)>2 : 
            ...
            break
    nch=len(uch)
    if id==0 or nch>35*35: return as2ch
    ...
    for i,x in enumerate(uch): as2ch[x]=ch2[i]
...
#for xyz
    ...
    nmodel=1
    if model and is_a_number(model[-1]) and int(model[-1])>1 : nmodel=int(model[-1])

    asym2ch = asym2chain(asym)
    
    nline=len(x)
    if not group : group=['ATOM' for i in x]
    if not alt : alt=['.' for i in x]
    if not ins : ins=['.' for i in x]
    if not natm : natm=['%d' %i for i in range(nline)]

    n=0
    for i in range(nline):
        nat=int(natm[i])
        nres=int(seq[i])
        if nat>99999 : nat=99999
        if nres>9999 : nres=9999
        
        if(len(symbol[i].strip())>1):
            tmp1="%s    " %atom[i].strip()
        else:
            tmp1=" %s    " %atom[i].strip()
        if (len(atom[i].strip()) == 4): tmp1 = "%s    "%atom[i]
        
        atomname=tmp1[:4]
        if nmodel>1 :
            if int(model[i])>=180 : break  #if too many atom, refmac breaks.
            if i==0:
                fw.write('MODEL %8s \n'%model[i])
        ...
        inst=ins[i]
        if ins[i]=='.' or  ins[i]=='?' : inst=" "
        
        alter=alt[i]
        if alt[i]=='.' or alt[i]=='?' : alter=" "
        v=[float(xx) for xx in ( x[i], y[i], z[i],occ[i], biso[i])]
        asym2=asym[i]
        if asym2ch : asym2=asym2ch[asym[i]]
        space=' '
        if v[4]<1000:
            ss='%-6s%5d %4s%1s%3s%2s%4d%1s   %8.3f%8.3f%8.3f%6.3f%6.2f%6s%-4s%2s  \n'\
 %(group[i],nat, atomname,alter,comp[i],asym2,nres, inst,v[0],v[1],v[2],v[3],v[4], space, asym[i], symbol[i])
        else:
...
#            print(n, ucomp[n], comp[i], uatom[n], atom[i],uasym[n], asym[i]) 
            if (n<len(ucomp) and ucomp[n]==comp[i] and uatom[n]==atom[i] and uasym[n]==asym[i] and useq[n]==seq[i]) :             
                ...
                ss='ANISOU%5d %4s%1s%3s%2s%4d%1s %7.0f%7.0f%7.0f%7.0f%7.0f%7.0f%2s%-4s%2s  \n' %(nat, atomname,alter,comp[i],asym2,nres,inst, u[0], u[1],u[2],u[3],u[4],u[5], space, asym[i], symbol[i]) 
                fw.write(ss)
                n=n+1
                
    if nmodel>1 : fw.write('ENDMDL  \n')
    fw.write('END\n')
    fw.close()
...
def get_xyz(flist):
    ...
    if not ins: ins=parse_values(items,values,"_atom_site.ndb_ins_code")
    alt=parse_values(items,values,"_atom_site.label_alt_id")
    model=parse_values(items,values, '_atom_site.pdbx_PDB_model_num')
    if not model : model=parse_values(items,values, '_atom_site.pdbx_model')
    if not model : model=parse_values(items,values, '_atom_site.ndb_model')
...
def add_twin(fw, flist):
    ...
    for i in range(ndomain):
        if domain_id : fw.write('REMARK   3      TWIN DOMAIN   :  %s\n' %(domain_id[i]))
        if operator : fw.write('REMARK   3      TWIN OPERATOR :  %s\n' %(operator[i]))
        if fraction : fw.write('REMARK   3      TWIN FRACTION :  %s\n' %(fraction[i]))
...
def add_tls(fw, flist):
    ...
    if not (tlsid and tlsid1): return
    ...
    if not (is_a_number(tlsid[-1]) and is_a_number(tlsid1[-1]) and
            tlsid[-1] == tlsid1[-1]):
    ...
    if bpart1 : fw.write('REMARK   3   ATOM RECORD CONTAINS RESIDUAL B FACTORS ONLY\n')
    ...
    for i in range(len(tlsid1)):
        ...
        if 'REFMAC' not in prog and i<nlen-1 :
            if tlsid1[i] == tlsid1[i+1] : continue
...
def add_header(fw,flist):
    ...
    if ids and type:
        for i,x in enumerate(type):
            if x=='split' : pdbids = pdbids  + ids[i] + ' '

    fw.write('HEADER    %-39s %16s    \n' %(name[0].upper(), pdbid[0].upper()))
    if len(pdbids)>2: fw.write('SPLIT      %s  \n'%pdbids) #legacy
...
REMARK   3  %s  
REMARK   3                                                                      
''' %(idd[0], resh[0],resl[0],comp[0],nobs[0],rwork[0],rfree[0],compfr[0],nfree[0],bres)
    fw.write(wds)
...
REMARK   3  %s  
REMARK   3                                                                      
''' %(idd[1], resh[1],resl[1],comp[1], nobs[1], rwork[1], rfree[1],compfr[1], nfree[1], bres)
...
# for 285    
    ...
    if text and 'FOLLOWING TRANSFORMATION MATRIX' in text[0]:
        t=text[0].split('\n')
        for x in t:
            if 'REMARK 285' not  in x and len(x)<72 : fw.write('REMARK 285 %-s\n' %x)
...
#for 350
    ...
    if items:
        ...
        if not (b11 and b12 and b13 and t1 and b21 and b22 and b23 and t2 and
                b31 and b32 and b33 and t3): return
        n=0
        for i in range(len(b11)):
            if btype and 'transform ' in btype[i] : continue
            n=n+1
            v=[float (x) for x in (b11[i],b12[i],b13[i],t1[i]) if is_a_number(x)]
            if len(v)==4: fw.write('REMARK 350   BIOMT1%4d %9.6f %9.6f %9.6f %14.5f \n' %(n, v[0],v[1],v[2],v[3]))
            v=[float (x) for x in (b21[i],b22[i],b23[i],t2[i]) if is_a_number(x)]
            if len(v)==4: fw.write('REMARK 350   BIOMT2%4d %9.6f %9.6f %9.6f %14.5f \n' %(n, v[0],v[1],v[2],v[3]))
            v=[float (x) for x in (b31[i],b32[i],b33[i],t3[i]) if is_a_number(x)]
            if len(v)==4: fw.write('REMARK 350   BIOMT3%4d %9.6f %9.6f %9.6f %14.5f \n' %(n, v[0],v[1],v[2],v[3]))
...
def add_ncs(fw,flist):
    ...
    if items:
        ...
        if not (id and code and b11 and b12 and b13 and t1 and b21 and b22 
                and b23 and t2 and b31 and b32 and b33 and t3): return
        for i in range(len(b11)):
            idd=' '
...
def add_remark200(fw,flist):
    ...
    if not syc : syc=parse_values(items,values,"_diffrn_source.ndb_synchrotron_y_n")
    if not syc : syc=['NULL']
    site=parse_values(items,values,"_diffrn_source.pdbx_synchrotron_site")
    if not site : site=parse_values(items,values,"_diffrn_source.ndb_synchrotron_site")
    if not site : site=['NULL']
    beam=parse_values(items,values,"_diffrn_source.pdbx_synchrotron_beamline")
    if not beam : beam=parse_values(items,values,"_diffrn_source.pdbx_synchrotron_beamline")
    if not beam : beam=['NULL']
    wave=parse_values(items,values,"_diffrn_source.pdbx_wavelength_list")
    if not wave : wave=parse_values(items,values,"_diffrn_source.rcsb_wavelength_list")
    if not wave : wave=['NULL']
...
REMARK 200                                                                      
''' %(syc[0],site[0].strip(),beam[0].strip(),wave[0].strip())
