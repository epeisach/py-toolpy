...
If add '-o output', user specify the output file name.
###############################################################################
...
def process(*files):
    ...
    for k in range(len(arg)):
        if(arg[k].upper() == "-PDB" or arg[k].upper() == "-F"):
            ...
            if n1:
                pdbfile=runmaxit(arg[k+1], 2, ' ')
            else:
                pdbfile=arg[k+1]
                
            if not os.path.exists(pdbfile):pdbfile=get_file_by_pdbid(pdbfile,1)
            
        elif(arg[k].upper() == "-DEP"):
            opt['dep']=1
            n1=is_cif(arg[k+1])
            if n1==1:
                dep, pdbdep = 1, runmaxit(arg[k+1], 2, ' ')
            elif n1==2:
                pdbdep=runmaxit(arg[k+1], 2, 'pdb_extract')
            else:
            ...
            if not os.path.exists(pdbdep): pdbdep=get_file_by_pdbid(pdbdep,1)
            ...
            pdbfile=arg[k+1]
            if is_cif(pdbfile) : pdbfile=runmaxit(arg[k+1], 2, ' ')
...
#---------------------------------------------------
    ...
    if os.path.exists('maxit.err') : delete_file('maxit.err')
...
#            print 'from refmac=',  t 
            ...
            t = parse_tls_range_phenix(tls_str, ntls, chain, ch_range)
            for y in t:
                y.append(ntls)
                all_range.append(y)
            #print 'from phenix=', t
    ...
    for x in all_range:
        ...
        if ch  not in chain.keys():
            t1='Error: chainID (%d) not in coordinate. TLS group=%s !' %(ch,nt)
            perror(t1)
    ...
    for i in range(0,nr):
        ...
        for j in range(i+1, nr):
            ...
            if ((n1<=k1 and n2>=k2) or     # 1
                ...
                t="Error! TLS group overlaps: group=%d, residue (%d  %d) with group=%d, residue (%d  %d) ; chainID=%s " %(nt1, k1, k2, nt2, k1,k2, ch)
                perror(t)
...
def get_xyz_tls(res, xyz):
    ...
    for x in xyz[:]:
        ...
        if (res[0] == x[21:22] and int(res[1]) <= int(x[22:26]) <= int(res[3])):
            nxyz.append(x)
    #        xyz.remove(x)
    return nxyz
...
def pdbtls_2cif(pdbfile):
    ...
    for i, x in enumerate(fp):
        ...
        if 'HEADER' in  x[:6] :
            ...
            tlsg = '%d' %ntls
            t = parse_tls_range_phenix(tls_str, tlsg, chain, ch_range)
            ...
            p = [xx, yy,zz]
              ...
              ('T11:' in x[14:20] and 'T22:' in x[29:34]) or
              ('T33:' in x[14:20] and 'T12:' in x[29:34]) or
              ('T13:' in x[14:20] and 'T23:' in x[29:34]) or
              ('L11:' in x[14:20] and 'L22:' in x[29:34]) or
              ('L33:' in x[14:20] and 'L12:' in x[29:34]) or
              ('L13:' in x[14:20] and 'L23:' in x[29:34]) 
...
def get_file_by_pdbid(pdbid_in, id):
    ...
    if ((id==1 and not os.path.exists(pdbfile)) or
        (id==2 and not os.path.exists(sffile))) : #try wget
        pdbfile = pdbid + ".pdb"
        sffile  = pdbid + "-sf.cif"
        if id==0:
...
def recover_anisou(anis, pdbfile, prog, outfile):
    ...
    if not check_file(50, tls_inp) : return
    ...
    for k, x in  enumerate(resid.keys()):
        ...
        if nn==1:
            ...
            for i, y in enumerate(resid[x]):
                ...
                if i != nn-1: str1 = str1 + ' OR '
            str1 = str1 + '))'

        if k  != kk-1: str1 = str1+ ' OR '
...
def make_tls_str(resid, id):
    ...
    if id=='buster':
        ...
        for k,v in resid.iteritems():
            for y in v : str1 = str1 + '%s|%d - %s|%d ' %(k, y[0],k, y[1])
...
#    pdb_sort = reorder_pdb(pdb_ref)
    ...
    if os.path.exists(ref2ph) and  os.path.exists(cif):
        tlscif=outfile + 'cif'
        shutil.move(ref2ph, outfile)
        shutil.move(cif, tlscif)
        
        print('The new output PDB file = %s' %outfile)
...
def map_tls_4_buster(pdbfile, pdbdep, outfile):
    ...
    print('Mapping tls using %s %s ' %(pdbfile,pdbdep))
#    pdb_ref = convert_tls2refmac(pdbfile, 'buster')
...
def convert_refmac2phenix(pdbfile):
    ...
    for i, x in enumerate(fp):
        ...
        elif ('NUMBER OF COMPONENTS GROUP' in x[12:41] or 'C SSSEQI' in x[31:41]
              or (len(x.strip())<12 and i<len(fp)-1 and 'TLS GROUP :' in fp[i+1])):
            continue
        
        elif 'RESIDUE RANGE :' in x[13:30]:
            res = parse_tls_range_refmac(x, ntls)
            if not res:
                print('Warning: Skipping residue range (%s) for TLS group (%s)'
                      %(ntls, x[30:].strip()))
                continue
...
def convert_tls2refmac(pdbfile, id):
    ...
    for x in fp:
        if ' TLS GROUP :' in x[11:24] :
        ...
        elif ' NUMBER OF TLS GROUPS  :' in x[11:37] :
            tt=x.split(':',1)[1].strip()
            if is_number(tt):
                ntls = int(tt)
                fw.write(x)
            ...
            for y in fp:
                ...
                if (str2 in y): #write in refmac foramt
                    if ( id=='phenix') :
                        tlsn = tls.strip().replace('(', ' ( ').replace(')', ' ) ')
                        tls_range = parse_tls_range_phenix(tlsn, tlsg, chain, ch_range)
                    elif (id=='buster') :
                        tlsn = tls.strip().replace('{', ' ').replace('}', ' ')
...
def parse_tls_range_buster_1(x, chain_range):
    ...
    if '*' in ch  and '*' in res: # all chains
        for k, v in chain_range.iteritems():
...
##########################################################
def keep_digit(x):
    '''only keep the digit and '-' and ':' from x
    '''

    t=filter(lambda y: y.isdigit() or y==':' or y=='-', x)
    return t
...
##########################################################
def parse_tls_range_phenix(tlsn, ng, chain, chain_range):
    ...
    s1a=myreplace_case('resi', 'resid', s1)
    s2=myreplace_case(' through ', ':', s1a)

#    s2=s1.replace('resseq', 'resid').replace(' through ', ':').strip()
    ...
    for i, x in enumerate(tmp):
        ...
        if len(t) != 1 and not is_number(t[0]) and t[0] != ':':
            tls.append(t.lower())
        else: #chid
            tls.append(t)
...
#            elif (pattern(i, tls, 'chain', '?') and (nn==2 or nn==4)) : #one chain
                  ...
                  (pattern(i, tls, 'chain', '?') and i>nn-3 and nn>5) or
                  (pattern(i, tls, 'chain', '?') and nn==2 ) or
                ...
                get_range_phe(chid, i+4, 1, chain_range, tls, tls_range)

            elif ((pattern(i, tls, 'chain', '?', 'and', '(', '(', 'res'))):
                get_range_phe(chid, i+5, 2, chain_range, tls, tls_range)
            ...
            if (chid not in chain_range.keys()): continue
            get_range_phe(chid, i+0, 1, chain_range, tls, tls_range)
...
##########################################################
def get_range_phe(chid, st, nb, chain_range, tls, tls_range):
    ...
    for m in range(st, nn):
        if (nb==1 and tls[m] == ')') or (nb==2 and tls[m] == ')' and tls[m+1] == ')' ):
            kk=1
            break
        if ':' in tls[m]:
#            x=tls[m]
    ...
    if kk==0:
        print('\nWarning: residue range possibly truncated! (need syntax check)' )
...
def finalize_pdb(pdbfile, outfile):
    ...
    if os.path.exists(newpdb1) : shutil.move(newpdb1, outfile)
...
def check_string_inpdb(pdbfile, *items_inp):
    ...
    if not os.path.exists(pdbfile): return 0
    fp=open(pdbfile, 'r')
    npdb=0
    for x in fp:
        if 'ATOM' in x[:4] or 'HETA' in x[:4] or 'ANISOU' in  x[:6] : continue
        for y in items[:]:
            if y in x[:len(y)]:
                items.remove(y)
                npdb=npdb+1
...
def sort_column_pdb(pdb, k1,k2, id):
    ...
    for ln in fp:
        ...
        if('ATOM' in ln[:4] or 'HETATM' in ln[:6] or 'ANISOU' in ln[:6]):
            ch=ln[21:22]
            d.append(ln)
            if id==1: cond = n==len(fp) 
            if id==2: cond = n==len(fp) or ch !=fp[n][21:22]
            if cond :
                d1=sort_column(d, k1, k2)
                for x in d1: fw.write(''.join(x))
                d=[]
...
def get_tlsinp(pdbfile):
    ...
    n=0
    for ln in fr:
        if ln[0:4]=='ATOM' or ln[0:6]=='HETATM' : break
...
def check_bfactor(pdbfile):
    ...
    if os.path.exists(pdbfile) and os.path.getsize(pdbfile) >1000 :
        ...
        for x in fr:
            ...
            if (x[0:4]=="ATOM" or x[0:4]=='HETA'): bval.append(float(x[60:66]))
            
        fr.close()
    n=len(negb)
    if bigb>0: print ('Error (%s): %d atom with B_full>999.' %(pdbfile,bigb))
    if n>0: print ('Warning (%s): (%d) atom with B_full<0.' %(pdbfile,n))
...
def separate_pdb_old(pdbfile):
    ...
    for x in fr1:
        if (('ATOM' in x[:4] or 'HETA' in x[:4]  or 'ANISOU' in x[:6]) and
            ('HOH' in x[17:20] or 'DOD' in x[17:20])):
            fw3.write(x)
        else:
            fr.append(x)
    ...
    for i in range(length):
        ...
        if (('CONECT' in fr[k][:6] or 'MASTER' in fr[k][:6]
             or 'END' in fr[k][:3] or 'HETATM' in fr[k][:6] or 'TER' in fr[k][:3] )
             and ('ATOM' in fr[k-1][:4] or 'TER' in fr[k-1][:3])) :
            n=k
            break
    if i> length-3 : n=length
    for i in range(length):
...
def replace_lines(pdbfile, rcsbpdb, str1, str2):
    ...
    if(n==0): print('Error! Nothing added to the pdb=' + pdbfile)
    return newpdb
...
def compare_two_pdb(pdbfile, pdbdep):
    ...
    if not (os.path.exists(pdbfile) and os.path.getsize(pdbfile)>100 and  
            os.path.exists(pdbdep)  and os.path.getsize(pdbdep)>100):
...
def match_pdb(tlsd,xx, pdb, dep,record, tmp):
    ...
    for n in range(n1,n2+1):
        if n not in record[ch].keys(): continue
...
def match_ligand_water(pdbfile, pdbdep):
    ...
    for i in range(m): #get correct component number
        n=m-i-1
        if(' RESIDUE RANGE :' in tlsd[n]): k=k+1
...
def check_and_correct_tls (pdbfile):
    ...
    xyzt=[]
    for x in fp:
        if atom_record(x) : xyzt.append(x)
    ...
    for x in fp:
        if 'REMARK   3   TLS GROUP :' in x :
            ...
            if pdb1 : write_tmp(pdbfile, fw, x, pdb1, res[0])
            if pdb2 : write_tmp(pdbfile, fw, x, pdb2, res[0])
            if pdb3 : write_tmp(pdbfile, fw, x, pdb3, res[0])
...
def write_tmp(pdbfile, fw, x, pdb1, ch):
    ...
    if ch not in chain_range1.keys() : return
...
def match_residue(ntls, chain1_range, xx, pdb, record_dep):
    ...
    for n in range(n1, n2+1):
        if n not in record_dep[ch].keys(): continue
    ...
    #chain, ch_range = chain_res_list(matched, 1) #all atoms
...
def write_resi_range(chain, ch_range, chain1_range, res, ntls):
    ...
    for c in sorted(ch_range.keys()):
        if c not in chain1_range: continue
        ...
        if (n1 != k1 or n2 != k2 or c != ch ):
            print('RESIDUE RANGE (group=%2s): %s -> [%s]' %(ntls, res, t2.strip()))
            
        if k1<poly1 and k2>poly2: #residue range span polymer
            ln=make_two_newline('!', chain[c], ln, c, poly1, poly2, 1)
            
        if (n2-n1 + 10 < k2-k1) : print('Warning: residue range change too much.',n2-n1, k2-k1 )
        new_range.append(ln)
...
def match_all_residues(pdbfile, pdbdep):
    ...
    record_dep = chain_res_atom(pdbd)
    ntls, tlsd_new = '?', []
    for x in tlsd : #through deposited TLS groups
        ...
        if 'REMARK   3   TLS GROUP :' in x :
            ...
            if len(tmp)>0:
                ...
                for x in tmp: 
                    if(len(x)>0): tlsd_new.append(x)

    m=len(tlsd_new)
    k=0
    for i in range(m): #get correct component number
        n=m-i-1
        if(' RESIDUE RANGE :' in tlsd_new[n]): k=k+1
...
def search_rcsb_dir(pdbfile, outfile, id):
    ...
    if id==1:
        ...
        for x in files:
            ...
            if wk==1: val=proc_tlsanl(pdb_match, outfile)
            if val==0 and wk==1 and cif in pdbdep:
                wk1, pdb_match = get_newtls(pdbdep, cif)
                if wk1==1: val = proc_tlsanl(pdb_match, outfile)
...
def get_deposited_file(rcsbid, id):
    ...
    pdb=p1 + '/' + rcsbid + '.pdb'
    p=p1
    if not os.path.exists(pdb): p=p2  #ndb dir
    if id==0:
        files.append(p)
        return files
    ...
    for x in val:
        ...
        if tmp in x or ('pdb' in x and 'rcsb' in x):
            files.append(x)
    if len(files)==0 : print('Warning: no extra files in dir=%s' %p)        
    return files
...
def get_local_file(x):
    ...
    if  '.cif' in pdbdep: pdbdep=runmaxit(pdbdep, 2, '-exchange_in')
    return pdbdep
    ...
    for ch in sorted(chain_range2.keys()):
        if ch not in chain_range1:  #write all
            ...
            for i, x in enumerate(fp2):
                if ch == x[21:22] :
                    if i>0 and fp2[i][22:26] !=  fp2[i-1][22:26] : nst = nst + 1
                    xx = x[:22] + '%4d' %nst + x[26:]
                    fw.write(xx)
                    #fp2.pop()
...
def new_ligand_water_residue_number(pdbfile):
    ...
    for x in chain2.values() :
        if len(x)>nn : nn=len(x)  #max number of ligand
    
    for x in pdb1 : fw.write(x) #write polymer first
    
    change_nres(fw, chain_range1, chain_range2, pdb2, 2)
    change_nres(fw, chain_range1, chain_range3, pdb3, 3+nn)
    
    for x in pdb4 : fw.write(x) #write tail
