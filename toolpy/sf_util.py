...
def sf_quality(sffile, pdbfile):
    ...
    for i,x in enumerate(flist):  #for xtriage/ctruncate
        t=x.strip()
        if ('<|L|>:' in t[:6] or '<L^2>:' in t[:8] or
            'L-test suggests' in t[:30] or 'twinning, twin fraction' in t):
            print(t)
        elif (t == 'Acentric reflections' or t == 'Centric reflections' or
            t == 'L test for acentric data'):
            print(t)
        elif (('<I^2>/<I>^2' in x  or '<F>^2/<F^2>' in x or ' <|E^2 - 1|> ' in x or 
              'Mean |L|   :' in x or 'Mean  L^2  :' in x or 'L statistic = ' in x ) and
              'untwinned' in x and ' perfect twin' in x):
            print(t)
...
def plot_items(plot_data, id, title,xlabel,ylabel,plot):
    ...
    if id =='twin':
        if len(title)==0: title='The Padilla-Yeates cumulative probability (acentric data)'
        if len(xlabel)==0: xlabel = '|L|'
        if len(ylabel)==0: ylabel =  'N(|L|)'
        if len(plot)==0: plot ='''  using 1:2 t "Observed data",  '' u 1:3 t "Untwinned by theory" ,  '' u 1:4 t "Twinned by theory" '''
    elif id == 'I/sigI' :
...
def extract_data4plot(fp, fname, start, end, idd):
    ...
    for y in fp:
        n=n+1
        if start in y and n<3 :continue
        if end in y and n>3 : break
        fw.write(y)
...
def get_file_type(inp1, inp2):
    ...
    if  id: # inp1 is SF file (cif or mtz)
        ...
        if (id): xyz_inp, sf_inp = inp1, inp2
...
def cif2cif_sf(flist, num):
    ...
    for i in range(0, n1): fw.write(flist[i]) #write the front part
    ...
    for i in range(len(rows)): 
        if int(rows[i][flag_col])==num and rows[i][status_col]=='o': rows[i][status_col]='f'

    cif.write_cif_loop(fw, items, rows)

    for i in range(n2, len(flist)): fw.write(flist[i]) #write the end part
...
def swap_sf(sffile):
    ...
    for line in fr:
        ...
        if  n>1000 and 'data_r' in line: break
