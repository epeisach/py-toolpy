...
#                print(s1)
            ...
            if 'HOH' in ln[17:20] or 'END' in ln[:3] : continue
            fo.write(ln)
            
#    fo.seek(0)
    for ln in wnew:  fo.write(ln)
    for ln in hoh:  fo.write(ln)
...
def rid_of_anisou(pdbfile):
    ...
    for ln in fr:
        if 'ANISOU' not in ln[:6]: fo.write(ln)
