...
#            test_cython.check_special_position(arg[k+1], outfile)
            ...
            args=[]
            for n in range(k+1, narg) : args.append(arg[n])
...
def find_xyzlimt(extend, coord):
    ...
    if not util.check_file(100, coord) : return xyzlim
    fp=open(coord,'r')
    for x1 in fp:
        if ('CRYST1' in x1[:6] ):
  ...
  #  print 'Finding blob of density (%s,%s) ...' %(coord, density)
    ...
    for x in fp1:
       ...
       # print x.strip()
        id=0
        for y in fp2:
            if 'ATOM' not in y[:4] and 'HETA' not in y[:4] : continue
        ...
        if id==0: blob.append(x)

       
    for x in blob: print(x.strip())
...
def gen_vtf_report(xyzf, sff, outf):
    ...
    cif=xyzf
    sf=sff
    if not util.is_cif(xyzf): cif=util.pdb2cif(xyzf) # a pdbfile
...
def calc_scale_matrix(pdbfile_in):
    ...
    pdbfile=pdbfile_in
    if be_cif: pdbfile =cif.cif2pdb(pdbfile_in)
...
#    vol = sg*sb*sa
    ...
    if be_cif: util.delete_file(pdbfile)
...
def run_superpose(k, arg, outfile):
    ...
    if not outfile: outfile='superpose.out'
    narg=len(arg)
    ss=''
    ...
    space=''
    for i, x in enumerate(fp):
        if 'Residue alignment:' in x: space='    '
        if not space:
...
#            print space , x.strip()
            ...
            if ':' in x :
                ...
                for y in ln :
                    if ':' in y :
                        t=y.split(':')
                        if not t[1] in aa: aa.append(t[1])


                if len(ln)>2 and i<len(fp)-1:
                    lln=fp[i+1].split() #the next line
                    if (util.is_number(ln[1]) and util.is_number(lln[1]) and int(ln[1])+1 !=int(lln[1])): brk='?'
