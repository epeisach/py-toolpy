...
def atom_site(flist):
    ...
    if not ins1: ins1=cif.parse_values(items,values,"_atom_site.ndb_ins_code")
    ...
    dic={'chain':chain, 'nres':nres, 'comp':comp, 'atom':atom, 'symbol':symbol,
         'alt':alt, 'ins':ins, 'x':x,'y':y, 'z':z,'biso':biso, 'occ':occ, 'natm':natm}
...
def get_value (res_rep, line):
    ...
    tmp=line.split()
    if '.' in tmp[2] : res_rep[0]=float(tmp[2])   #resh
    if '.' in tmp[3] : res_rep[1]=float(tmp[3])   #resl
    if '.' in tmp[5] : res_rep[2]=float(tmp[5])   #Rwork
    if '.' in tmp[4] and '?' in tmp[5] : res_rep[2]=float(tmp[4])  #Rwork
    if '.' in tmp[6] : res_rep[3]=float(tmp[6])   #Rfree
    if '.' in tmp[8] : res_rep[4]=float(tmp[8])   #Comp
    if '.' in tmp[10]: res_rep[5]=float(tmp[10])  #FCC
    if '.' in tmp[11]: res_rep[6]=float(tmp[11])  #Real_R
    if '.' in tmp[12]: res_rep[7]=float(tmp[12])  #Dcc
