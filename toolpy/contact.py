...
import util, space_group_cif, parse, cifparse as cif
            ...
            for kk in box:
                for j in range(nop): #number of operators
                    ...
                    for i in range (nxf):
                        ...
                        if (xnf0<xmin[0] or xnf0>xmax[0] or xnf1<xmin[1] or xnf1>xmax[1] or
                            xnf2<xmin[2] or xnf2>xmax[2]) : continue
                    ...
                    #    xno = util.matrix_prod(orth,[xnf0,xnf1,xnf2])
                        ...
                        if d<0.5 and (j!=0 or '555' not in scode) : # atom on special position
                            s1='%s_%s_%s_%s_%s_%s_%s_%d' %(info['chain'][i], info['comp'][i],
...
def check_coord_occ(info, id, nfold):
    ...
    ss='?'
    atom='(%s : id=%s_%s_%s)' %(info['atom'][na], info['chain'][na],
                                info['comp'][na], info['nres'][na])
...
def update_coord(file, swater, occ, out_xyz):
    ...
    for i,x in enumerate(flist): #get the first, part.
        if ('loop_' in x[:5] and '_atom_site.' in flist[i+1][:11] and
            '_atom_site.' in flist[i+2][:11]): 
            n1=i
            break
    ...
    for i in range(len(rows)): 
        for j, x in enumerate(swater):
            if (rows[i][ch_col]==x[0] and rows[i][comp_col]==x[1] and
                rows[i][nres_col]==x[2] and rows[i][atom_col]==x[3] and
                rows[i][alt_col]==x[4] and rows[i][ins_col]==x[5] and
                rows[i][natm_col]==x[6] ):
...
def shift_xyz2unit(x,y,z):
    ...
    if xc==0 and yc==0 and zc==0 : return x,y,z
    xn,yn,zn=[],[],[]
    for i in range(n):
        x1=x[i] - xc
        y1=y[i] - yc
...
def get_atominfo(file):
    ...
    atominfo={'chain':[], 'nres':[], 'comp':[], 'atom':[],'symbol':[],
              'alt':[], 'ins':[],'x':[],'y':[],'z':[],'xf':[],'yf':[],'zf':[],
              'occ':[], 'mtr':[], 'symop':[],'spg':'','cell':[],'mtrix':[],
              'atom_o':[], 'group':[], 'natm':[], 'natm_c':[], 'segid':[]}
...
def data_from_pdb(file, info):
    ...
    info={'chain':chain, 'nres':nres, 'comp':comp, 'atom':atom, 'biso':biso,
          'occ':occ, 'symbol':symbol, 'alt':alt,'ins':ins,'x':x,'y':y,
          'z':z,'xf':xf,'yf':yf,'zf':zf,'mtr':mtr, 'symop':symop, 'spg':spg,
          'cell':cell, 'mtrix':mtrix, 'group':group, 'natm':natm, 'natm_c':natm_c,
          'atom_o':atom_o, 'segid':segid}
...
#    return
    ...
    print('xmin, xmax =(%.2f %.2f): ymin, ymax =(%.2f %.2f): \
    zmin, zmax =(%.2f %.2f)' %(xmin, xmax, ymin, ymax, zmin, zmax))
...
#    for x in grid.keys(): print x, grid[x]
    ...
    for xx in grid.keys():  #non-empty
        if not grid[xx] : continue
        t=xx.split('_')
        i, j, k = int(t[0]), int(t[1]), int(t[2])
        cell_0, cell_26=[],[]
        for ii in (-1, 0, 1): #search neighbor atoms
            n1=i + ii
            if n1 <0 or  n1 >nx : continue
            for jj in (-1, 0, 1):
                n2=j + jj
                if n2 <0 or n2 >ny : continue
                for kk in (-1, 0, 1):
                    n3=k + kk
                    if n3 <0 or n3 >nz : continue
                    tt='%d_%d_%d' %(n1,n2,n3)
                    
                    if not grid[tt]: continue   #remove empty cell. 
        ...
        for ijk in cell_0: atom_cell_0.extend(grid[ijk])
        for ijk in cell_26: atom_cell_26.extend(grid[ijk])

        if idd==0: #atom contact
            dis=check_atom(chain, nres, comp, atom, symbol, alt, ins,
                           x,y,z,mtr, atom_cell_0, atom_cell_0,natom) 
            alldis.extend(dis)
            dis=check_atom(chain, nres, comp, atom, symbol,alt, ins,
                           x,y,z,mtr, atom_cell_0, atom_cell_26,natom) 
            alldis.extend(dis)
            
        elif idd==1: #alt
            dis=check_atom_alt(chain, nres, comp, atom, symbol, alt, ins,
                               x,y,z,mtr, atom_cell_0, atom_cell_0,natom) 
            alldis.extend(dis)
            dis=check_atom_alt(chain, nres, comp, atom, symbol,alt, ins,
                               x,y,z,mtr, atom_cell_0, atom_cell_26,natom) 
    ...
    for n,xx in enumerate(alldis): #get rid of the duplicate
        if n>0 and xx[1]== alldis[n-1][2]: continue
        if '_CYS_SG_' in xx[1] and '_CYS_SG_' in xx[2] and 1.7<xx[0]<2.3 : continue 
    
        netdis.append(xx)
        

    if idd==1: return netdis #alt
...
def check_atom_pair(m,n,atom, d):
    ...
    elif (((atom[n]=='CA' and atom[m]=='C' or atom[m]=='CA' and atom[n]=='C') and 2.2<d<2.6) or
          ((atom[n]=='N' and atom[m]=='CA' or atom[m]=='N' and atom[n]=='CA') and 2.2<d<2.6) or
          ((atom[n]=='N' and atom[m]=='O' or atom[m]=='N' and atom[n]=='O') and 1.9<d<2.5)):
    ...
    elif ((((atom[n]=='OP1' or atom[n]=='OP2') and (atom[m]=="O3'" or atom[m]=="O3*") or
            (atom[m]=='OP1' or atom[m]=='OP2') and (atom[n]=="O3'" or atom[n]=="O3*") and 2.0<d<2.6) or
           
           ((atom[n]=="O3'" or atom[n]=="O3*") and (atom[m]=="O5'" or atom[m]=="O5*") or
            (atom[m]=="O3'" or atom[m]=="O3*") and (atom[n]=="O5'" or atom[n]=="O5*") and 2.4<d<3.2) or
           
           ((atom[n]=="C3'" or atom[n]=="C3*") and (atom[m]=="P" ) or
            (atom[m]=="C3'" or atom[m]=="C3*") and (atom[n]=="P" ) and 2.2<d<2.8) )) :
        return 1  #bond angled atoms
...
def check_atom(chain, nres, comp, atom,symbol, alt, ins, x,y,z, mtr, atom_cell_1,atom_cell_2,natom):
    ...
    for n in atom_cell_1:
        for m in atom_cell_2:
            if abs(x[n]-x[m])>cutoff or abs(y[n]-y[m])>cutoff or abs(z[n]-z[m])>cutoff : continue
            n1=nres[n] #residue number
            n2=nres[m]
            if mtr[-1] > mtr[0]: #matrix exist! Only check matrix generated atoms!
                if mtr[n] == mtr[m] : continue 
            else:
                if (n1==n2 and chain[n]==chain[m] and ins[n]==ins[m] ): #same residue
                    ...
                    if abs(n-m) <18 and check_atom_pair(n,m,atom,d) : continue
                elif (chain[n]==chain[m] and abs(n1-n2)==1): 
                    d=math.sqrt((x[n]-x[m])**2 + (y[n]-y[m])**2 + (z[n]-z[m])**2)
                    if check_atom_pair(n,m,atom,d) :continue
                elif ((0<n<natom-1 and nres[n]!=nres[n-1] and nres[n]!=nres[n+1]
                       and (comp[n] !='HOH' and comp[n] !='DOD')) or
                      (0<m<natom-1 and nres[m]!=nres[m-1] and nres[m]!=nres[m+1]
                       and (comp[m] !='HOH' and comp[m] !='DOD'))): continue #sigle atom     
            d=math.sqrt((x[n]-x[m])**2 + (y[n]-y[m])**2 + (z[n]-z[m])**2)
            if( symbol[n]=='H' or symbol[n]=='D' or symbol[m]=='H' or symbol[m]=='D') and d>1.5 : continue
            ...
            if d<cutoff:  alldis.append([d, atom1, atom2])
...
def check_atom_alt(chain, nres, comp, atom,symbol, alt, ins, x,y,z, mtr, atom_cell_1,atom_cell_2,natom):
    ...
    for n in atom_cell_1:
        for m in atom_cell_2:
            ...
            n1=nres[n] #residue number
            n2=nres[m]
            if not (n1==n2 and atom[n] == atom[m]) :continue
    ...
    for n in range(len(x)):
        for m in range(len(x)):
            ...
            if (m==n or (chain[n]==chain[m] and (n1-n2<1 or n2-n1<1))):
                ...
                d=math.sqrt((x[n]-x[m])**2 + (y[n]-y[m])**2 + (z[n]-z[m])**2)
                if d<2.3:
                    print('d=%.2f : atom1=%s_%s_%s_%s_%s_%s : atom2=%s_%s_%s_%s_%s_%s' \
                          %(d, chain[n], nres[n], comp[n], atom[n], alt[n], ins[n], \
                            chain[m], nres[m], comp[m], atom[m], alt[m], ins[m]))

##################################################################
def check_atom_fes(chain, nres, comp, atom,symbol, alt, ins, x,y,z,
                   mtr, atom_cell_1,atom_cell_2,natom):
        ...
        for m in atom_cell_2:
            ...
            if mtr[-1] > mtr[0]: #matrix exist! Only check matrix generated atoms!
                ...
                if ((('FES' in comp[n] and 'FES' in comp[m]) or ('FES' in comp[m] and 'FES' in comp[n]))  and
                    (('S' in atom[n] and atom[m][0]=='S') or ('S' in atom[m][0] and 'S' in atom[n][0]) or 
                    ('FE' in atom[n] and 'FE' in atom[m]) or ('FE' in atom[m] and 'FE' in atom[n]))):
                    d=math.sqrt((x[n]-x[m])**2 + (y[n]-y[m])**2 + (z[n]-z[m])**2)
                    
                    atom1='%s_%s_%s_%s_%s_%s_%d' %(chain[n], nres[n], comp[n], atom[n], alt[n], ins[n],mtr[n])
                    atom2='%s_%s_%s_%s_%s_%s_%d' %(chain[m], nres[m], comp[m], atom[m], alt[m], ins[m],mtr[m])
                    if 0.1<d<3.5:  alldis.append([d, atom1, atom2])
                    
                elif(('FES' in comp[n] or 'FES' in comp[m]) and ('FE' in atom[n] or 'FE' in atom[m])  and n1!=n2):
                    if  abs(x[n]-x[m])>4 or abs(y[n]-y[m])>4 or abs(z[n]-z[m])>4 : continue
                    d=math.sqrt((x[n]-x[m])**2 + (y[n]-y[m])**2 + (z[n]-z[m])**2)
            ...
            for jj in (-1, 0, 1):
                for kk in (-1, 0, 1):
                    ...
                    for j in range(len(m1)): 
                        ...
                        if (xno[0]<xmin or xno[0]>xmax or xno[1]<ymin or xno[1]>ymax
                            or xno[2]<zmin or xno[2]>zmax) : continue
                        #print xo, xno
                        ...
                        nop=j
                        print('ATOM  %5d %4s%1s%3s %1s%4s%1s   %8.3f%8.3f%8.3f%6.2f%6.2f%3d %s %.3f' %( 1,info['atom'][i],info['alt'][i],info['comp'][i],info['chain'][i], info['nres'][i],info['ins'][i],xno[0],xno[1],xno[2],  0,0, info['mtr'][i],scode, d))
