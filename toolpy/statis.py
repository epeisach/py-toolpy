...
def histogram_plot(infile, xcol, xlabel, ylabel, title, nstep, xxrange):
    ...
    for x in flist:
        t=x.split()
        if xcol > len(t) or not util.is_number(t[xcol-1]): continue
...
#    bx1, bx2 = limits_by_iqr_median(data_o, col, scale)
    ...
    if xxrange: bx1,bx2=get_range(xxrange)
    ...
    if nstep<=0 : nstep=20 
    if not title: title = 'Data distribution (entry=%d)' %(len(data_n))
    if not xlabel : xlabel='data (column=%d)'%xcol
    if not ylabel : ylabel='Frequency'
...
def correlation_plot(infile, xcol, ycol, xlabel,ylabel,title,nstep, xxrange, yyrange):
    ...
    for x in flist:
        t=x.split()
        if (len(t) <xcol or len(t) <ycol or (not util.is_number(t[xcol-1])) 
            or (not util.is_number(t[ycol-1]))): continue
    ...
    if xxrange: bx1,bx2=get_range(xxrange)
    if yyrange: by1,by2=get_range(yyrange)

    data_n=[]  #filtered data
    for x in data_o:
        if bx1<=x[0]<=bx2 and by1<=x[1]<=by2 : data_n.append (x) 
    ...
    if not xlabel : xlabel='data (column=%d)'%xcol
    if not ylabel : ylabel='data (column=%d)'%ycol
    ...
    plot="""plot '%s' using 1:4:3:7:6:xticlabels(2) with candlesticks title 'Quartiles' whiskerbars, \
...
#    data_corr(fcorr,data_o, shell, 0, '')
    ...
    plot = "\nf(x) = a*x + b\n fit f(x) '%s' u %d:%d via a, b\n title_f(a,b) = sprintf('f(x) = %%.2fx + %%.2f', a, b) \nplot '%s' using %d:%d , f(x) t title_f(a,b) " %( infile, xcol, ycol, infile, xcol, ycol)
    ...
    if os.path.exists('fit.log'):
        ...
        fp=open('fit.log', 'r')
        for x in fp:
            if 'a ' in x[:3] and ' +/- ' in x and '(' in x: a=x 
            if 'b ' in x[:3] and ' +/- ' in x and '(' in x: b=x 
        print("%s %s" % (a, b))
...
def scatter_plot(file, col1, col2):
    ...
    for x in flist:
        ...
        if col1 >= len(t) or  col2 >= len(t)  : 
            ...
            continue
        if not util.is_number(t[col1]) or not util.is_number(t[col2]): continue
...
def limits_by_iqr_median(datain, col, scale):
    ...
    for z in datain:  #use the raw data
        if z[col] <= maxi: data1.append(z)
        if z[col] >= maxi: data2.append(z)
...
def get_median(datain, col):
    ...
    if col>=0: #
        data=[]
        for x in datain: data.append(x[col])
...
def get_mc(datain, col):
    ...
    if col<0: #single list
        for z in datain:  
            if z <= med : data1.append(z)
            if z >= med : data2.append(z)
    else: #list of list 
        for z in datain:  
            if z[col] <= med : data1.append(z[col])
            if z[col] >= med : data2.append(z[col])
...
plot sin(x) lt -1
    ...
    if(len(xxrange)>1) : fw.write("set xrange %s\n" %xxrange)
...
def add_percentile_rank(infile, coln):
    ...
    for x in flist:
        ss=x.strip().split()
        if  not ss or len(ss)<coln or not ss[0].isdigit or  '?' in ss[col] : continue
        ulist.append(ss[col])
