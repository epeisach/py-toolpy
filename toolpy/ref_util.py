...
def remove_allalt_conformer(file):
    ...
    for i, x in enumerate(flist):
        if (('ATOM' in x[:4] or 'HETATM' in x[:6] or 'ANISOU' in x[:6])
            ...
            for j in range(i+1, n):
                y=flist[j]
                if x[22:26] != y[22:26] : break
                if (x[11:16]==y[11:16] and x[17:26]==y[17:26] and
                    x[16:17] != y[16:17] and y[16:17] != ' '): tmp.append(j)
    ...
    for i, x in enumerate(flist):
        if (('ATOM' in x[:4] or 'HETATM' in x[:6] or 'ANISOU' in x[:6])
            ...
            t=x
            if ('ATOM' in x[:4] or 'HETATM' in x[:6]): t=x[:54] + '  1.00' + x[60:]
            t1=t[:16] + ' ' + t[17:]
            fw.write(t1)
...
def add_water(mtz, pdbfile, id):
    ...
    for x in fp:
        if 'ATOM' in x[:4]:
            natom=n+na
            nres=n+nr+1
            if natom>99999 : natom=99999
            if nres>9999 : natom=9999
            if float(x[54:60]) >6:
...
END-watpeak
    ''' %(map, peak, rms, peak, pdbfile, npdb, symm)
...
''' %(maptype,f1,phi,  mtz,  f1,phi,  pdbfile,mapout)
...
#    bavg=0
    for i, x in enumerate(fp):
        if (i>0 and ('ATOM' in fp[i-1][:4] or 'HETATM' in fp[i-1][:6]) and
            'ANISOU' in x[:6] and (len(x)<77 or x[77:78]==' ')):
            t=x[:70]+ '      '+ fp[i-1][76:80] +'\n' 
            fw.write(t)
