...
def process(*files):
    ...
    for k in range(len(arg)):
        if(arg[k].upper() == "-PDB" or arg[k].upper() == "-F"):
            ...
            if is_cif(arg[k+1]) : pdbfile = cif2pdb(arg[k+1])
            ...
            pdbfile=arg[k+1]
            if is_cif(pdbfile) : pdbfile= cif2pdb(arg[k+1])
            cif = tls2cif(pdbfile)
            out='%s_tls2cif' %arg[k+1]
            os.system('mv -f %s %s' %(cif, out))
    ...
    for i, x in enumerate(fp):
        ...
        if 'HEADER' in  x[:6] :
            ...
            p = [xx, yy,zz]
              ...
              ('T11:' in x[14:21] and 'T22:' in x[25:]) or
              ('T33:' in x[14:21] and 'T12:' in x[25:]) or
              ('T13:' in x[14:21] and 'T23:' in x[25:]) or
              ('L11:' in x[14:21] and 'L22:' in x[25:]) or
              ('L33:' in x[14:21] and 'L12:' in x[25:]) or
              ('L13:' in x[14:21] and 'L23:' in x[25:]) 
...
def convert_refmac2phenix(pdbfile):
    ...
    out=pdbfile + '_ref2ph'
    if not check_file(50, pdbfile) : return out
...
def sort_column_pdb(pdb, k1,k2, id):
    ...
    for ln in fp:
        ...
        if('ATOM' in ln[:4] or 'HETATM' in ln[:6] or 'ANISOU' in ln[:6]):
            ...
            if id==2: cond = n==len(fp) or ch !=fp[n][20:22].strip()
            if cond :
